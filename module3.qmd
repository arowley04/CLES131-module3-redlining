...
balt_bbox <- st_bbox(balt_full) # get bounding box in UTM meters

# Overlay projected vector file onto projected NDVI 
ggplot() +
  geom_spatraster(data = balt_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale",
  ...
  coord_sf(crs = st_crs(balt_full), # forces 32618 (meters) bc it kept being in coordinates
           xlim = c(balt_bbox["xmin"], balt_bbox["xmax"]),
           ylim = c(balt_bbox["ymin"], balt_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(balt_full)) +
  theme_minimal() +
  labs(title = "Baltimore, MD")
```

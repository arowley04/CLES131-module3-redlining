---
title: "Module 3: Redlining"
format: pdf
editor: source
editor_options: 
  chunk_output_type: console
---

```{r}
#| warning: false
library(sf) # simple features for R
# install.packages("terra")
library(terra) # spatial data analysis
library(tidyterra) #tidyverse methods for terra objects
library(tidyverse)
library(dplyr)
```

### Use of GitHub

Link to your forked GH repository: git@github.com:arowley04/CLES131-module3-redlining.git

### Use of Quarto

Link to your .qmd file: 

# Ecological consequences of redlining

In August 2020, [Christopher Schell](https://cjschell.com/about) and collegues published a review in *Science* on ['The ecological and evolutionary consequences of systemic racism in urban environments'](https://science.sciencemag.org/content/early/2020/08/12/science.aay4497) showing how systematic racism and classism has significant impacts on ecological and evolutionary processes within urban environments. Here, we combine spatial data to reproduce and extend an analysis from the paper.

## The vector data

We will use a vector dataset of redlining maps from [Mapping Inequality](https://dsl.richmond.edu/panorama/redlining), a project led by [Robert K. Nelson](https://americanstudies.richmond.edu/faculty/rnelson2/).

### Q1 (1 point)

Click 'Explore the Maps' to look at some cities and neighborhoods you are familiar with. Who is the intended audience of this data science project, and how are the data used to communicate understanding, insight, and knowledge? Why is this effective?

I chose Chicago, IL. I assume the intended audience of this data science project is modern day individuals interested in learning about the history of redlining in America. The map of Chicago created using the 1940 census data was likely intended for real estate developers, city planners, policy makers, investors/banks, and other individuals/organizations that played a role in redlining Chicago back in the mid 1900's. 

The polygons on the map represent different property types and categorized by most to least desirable for residential areas, which is denoted by color, and industrial and commercial areas are denoted by symbol. The color choice for assigning implied value to residential areas green (best), blue (still desirable), yellow (definitely declining), and red (hazardous) is effective because these colors have the same intuitive meaning to a diverse audience and can be easily identified on the map. 

The 'Note About This Data' describes how racial profiles were accounted by grouping different minorities together based on the 1940 city census, which is beneficial for users today to understand how the raw data was manipulated and represented on the map. This section suggests some potential bias in the representation of certain Asian demographics. Additionally, the "Context" and "Area Descriptions" provide a lot of useful background about the history of redlining in Chicago and the racial demogaphics within different delineated areas. 

### Q2 (1 point)

Create a `data/` folder in the root of your project and create five subfolders labeled with the city names from Fig. 2 of Schell et al. 2020. Because the spatial files will be large, add `data/` to the .gitignore file. 

Then, go back to the home page of [Mapping Inequality](https://dsl.richmond.edu/panorama/redlining) and select 'Download the Data'. Use the search bar to select and download spatial data for each city. Move the geojson file into the associated data subfolder. 

Import the geojson file into your R environment with the `st_read`()` function from sf. Check the structure of this object and see that it is a special type of data frame, allowing it to be manipulated with many of the functions you already know, including ggplot. 

Make a quick plot of your first city showing the "grade" in color using ggplot syntax and `geom_sf()`. Select a color scheme that better comports with redlining. 

```{r}
# Read in data 
baltimore <- st_read("data/baltimore/balt_geojson.json") 
baltimore  
st_crs(baltimore)

birmingham <- st_read("data/birmingham //birm_geojson.json") 
birmingham 
st_crs(birmingham)

indianapolis <- st_read("data/indianapolis//indy_geojson.json") 
indianapolis
st_crs(indianapolis)

minneapolis <- st_read("data/minneapolis/minn_geojson.json") 
minneapolis
st_crs(minneapolis)

pheonix <- st_read("data/pheonix/pho_geojson.json") 
pheonix
st_crs(pheonix)
```

```{r}
# Plot first city

# unique(baltimore$fill) #see original hex code for each "grade" in color
# baltimore[baltimore$fill == "#00000", ] #inspecting weird NA values
baltimore$grade[is.na(baltimore$grade)] <- "Industrial & Commercial" #assigning NA values as industrial/commercial areas

ggplot() +
  geom_sf(data = baltimore, aes(fill = grade)) +
  scale_fill_manual(values = c("A"="#76a865", 
                               "B"="#7cb5bd", 
                               "C"="#ffff00", 
                               "D"= "#d9838d",
                               "Industrial & Commercial"= "lightgrey"),
                    labels = c("A" = "Best",
                               "B" = "Still Desireable",
                               "C" = "Definitely Declining",
                               "D" = "Hazardous",
                               "Industrial & Commercial" = "Industrial & Commercial")) + 
  labs(title = "Baltimore, MD (1940) Redlining Map",
       subtitle = "GCS WGS84",
       fill = "Grade") +
  theme_bw()
```


## The raster data

We will also be calculating NDVI from the European Space Agency's [Sentinel-2 Mission](https://documentation.dataspace.copernicus.eu/Data/SentinelMissions/Sentinel2.html), specifically bands B4 (red) and B8 (near infrared). There are multiple steps to importing the data, which itself takes a long time, so please get an early start.

 - Click "Explore Sentinel-2 data" on this [page](https://dataspace.copernicus.eu/data-collections/copernicus-sentinel-data/sentinel-2) and create an account to login
 - In the explorer, make sure Sentinel-2 L2A is selected (Level 2A, atmospheric correction applied)
 - Scroll and zoom to the city of choice
 - Use the polygon tool (upper right corner, hover over pentagon icon and select rectangle) to draw a bounding box. Adjust until the extent approximates those in Schell et al. 2020. Try selecting the "False color" layer to help diagnose features to include or exclude
 - Set a threshold for cloud cover and select a date that reasonably approximates peak greenness. You may have to test multiple options to locate it, and not all cities will have the same date
 - Once the displayed images looks satisfactory, click "Find products for current view"
 - Check the desired product and download. It will take a while because the files are large
 
The data will be packaged as a zipped SAFE file in your Downloads folder. You may need to investigate the properties of the file and click 'unblock' to give permission to open. Once unzipped, you will find:
 - The images are jpeg2000 files nested within the GRANULE and IMG_DATA subfolders
 - Multiple resolutions and bands are available
 - Metadata is provided in `MTD_MSIL2A.xml`

For each city, locate the 10m resolution files for the B04 and B08 bands along with associated metadata and copy them to `data/city_name/` within this project. 

### Q3 (1 point)
Use the terra package and the `rast()` function to import the two bands, which are reported as digital numbers. 
Combine to calculate NDVI ($ (NIR - R) / (NIR + R)$) and display a quick plot of your first city. 

```{r}
# Read in data 
#note: data was provided by Adam bc I had so much trouble accessing the files and started quite late. I confirmed this with you earlier last week.  
balt_R <- rast("data/baltimore/T18SUJ_20250603T155001_B04_10m_copy.jp2")
balt_NIR <- rast("data/baltimore/T18SUJ_20250603T155001_B08_10m_copy.jp2")
terra::crs(balt_R)
terra::crs(balt_NIR)

birm_R <- rast("data/birmingham /T16SEC_20190831T162839_B04_10m.jp2") 
birm_NIR <- rast("data/birmingham /T16SEC_20190831T162839_B08_10m.jp2") 

indy_R <- rast("data/indianapolis/T16TEK_20251003T163141_B04_10m.jp2")
indy_NIR <- rast("data/indianapolis/T16TEK_20251003T163141_B08_10m.jp2")

minn_R <- rast("data/minneapolis/T15TVK_20190808T170901_B04_10m.jp2")
minn_NIR <- rast("data/minneapolis/T15TVK_20190808T170901_B08_10m.jp2")

phe_R <- rast("data/pheonix/T12SUC_20221118T181641_B04_10m.jp2")
phe_NIR <- rast("data/pheonix/T12SUC_20221118T181641_B08_10m.jp2")
```

```{r}
# Calculate NDVI
balt_NDVI <- (balt_NIR - balt_R) / (balt_NIR + balt_R)
balt_NDVI
crs(balt_NDVI)


birm_NDVI <- (birm_NIR - birm_R) / (birm_NIR + birm_R)

indy_NDVI <- (indy_NIR - indy_R) / (indy_NIR + indy_R)

minn_NDVI <- (minn_NIR - minn_R) / (minn_NIR + minn_R)

phe_NDVI <- (phe_NIR - phe_R) / (phe_NIR + phe_R)

# Quick plot of first city 
plot(balt_NDVI)
```

### Bonus 1 (1 point)
Since you have 5 cities to plot, can you optimize the same operations as Q3 with a for loop? 

### Q4 (1 point)
Do the rasters and polygons share the same coordinate reference system? If not, project both into the same CRS and justify your choice. 

They do not share the same coordinate reference system. The polygon data (.json files) uses the geographic coordinate system (WGS 84 (EPSG 4326)), while the raster data (.jp2 files) use a projected coordinate system (WGS 84 / UTM zone 18N (EPSG:32618).

```{r}
# Project both into the same CRS
balt_full <- st_transform(baltimore, 
                          crs = terra::crs(balt_NDVI)) 
# balt_full

birm_full <- st_transform(birmingham, 
                          crs = terra::crs(birm_NDVI)) 


indy_full <- st_transform(indianapolis, 
                          crs = terra::crs(indy_NDVI)) 


minn_full <- st_transform(minneapolis, 
                          crs = terra::crs(minn_NDVI)) 


phe_full <- st_transform(pheonix,
                         crs = terra::crs(phe_NDVI)) 
```

Map projections come in 2 varieties: 
1. Conformal - preserves angles but not lengths
2. Equal area projections - area is preserved but shapes or bearings are distorted 
It's not possible to 100% preserve both. Maps require compromises to best illustrate specific features based on the goals of the map. In this case, I wanted to preserve the raster data because it already had a projected coordinate system containing accurate distance and area measurements (m) for the area, so it makes sense to keep this layer and reproject the polygon data. 

### Q5 (1 points)
Overlay the projected vector file onto the projected NDVI for a single city using `tidyterra::geom_spatraster()` in addition to `geom_sf()`. Adjust the color scales and add a scale bar to approximate Fig. 2a of Schell et al. 2020. 

```{r}
# Plot Baltimore 
#install.packages("ggnewscale")
library(ggnewscale) #adds multiple scales
library(ggspatial) #adds annotation scale


balt_bbox <- st_bbox(balt_full) # get bounding box in UTM meters https://www.rdocumentation.org/packages/sp/versions/2.2-0/topics/bbox-methods

# Overlay projected vector file onto projected NDVI 
p2 <- ggplot() +
  geom_spatraster(data = balt_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale", #use when there's no given scale
                       colours = c("#36454f", "white", "#006400"),
                       limits = c(-1, 1),
                       breaks = c(-0.5, 0, 0.5, 1),
                       labels = c("Low NDVI\n(low vegetation)", "", "", 
                                  "High NDVI\n(high vegetation)"),
                       na.value = "transparent") +
  guides(fill = guide_colorbar(reverse = TRUE)) + #change order scale appears in
  new_scale_color() + #for redlining scale
  geom_sf(data = balt_full, 
          aes(color = grade), 
          fill = NA, #make outlines for redline areas
          linewidth = 1) +
  scale_color_manual(name = "Redlining Grade",
                     values = c("A" = "#76a865",
                                "B" = "#7cb5bd",
                                "C" = "#ffff00",
                                "D" = "#d9838d",
                                "Industrial & Commercial" = "lightgrey"),
                     guide = guide_legend(override.aes = list(fill = NA, size = 2))) + #make square outlines 
  annotation_scale(location = "bl",
                   width_hint = 0.25,
                   unit_category = "imperial", #converts to miles
                   text_cex = 0.8,
                   bar_cols = c("black", "white")) +
  coord_sf(crs = st_crs(balt_full), # forces meters bc it kept being in coordinates
           xlim = c(balt_bbox["xmin"], balt_bbox["xmax"]), #https://r-spatial.github.io/sf/reference/st_bbox.html 
           ylim = c(balt_bbox["ymin"], balt_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(balt_full)) +
  theme_minimal() +
  labs(title = "2. Baltimore, MD") +
  theme(plot.title = element_text(size = 8.5, 
                                  face = "bold"))
p2
```


### Q6 (1 point)
Repeat the above for all 5 cities and add the city name. Explore the `cowplot` or `patchwork` packages to create a multi-panel figure. 
```{r}
### Repeat the above for all 5 cities and add the city name.

## Indianapolis, IN
indy_bbox <- st_bbox(indy_full) # get bounding box in UTM meters

# Overlay projected vector file onto projected NDVI 
p1 <- ggplot() +
  geom_spatraster(data = indy_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale", #use when there's no given scale
                       colours = c("#36454f", "white", "#006400"),
                       limits = c(-1, 1),
                       breaks = c(-0.5, 0, 0.5, 1),
                       labels = c("Low NDVI\n(low vegetation)", "", "", 
                                  "High NDVI\n(high vegetation)"),
                       na.value = "transparent") +
  guides(fill = guide_colorbar(reverse = TRUE)) + #change order scale appears in
  new_scale_color() + #for redlining scale
  geom_sf(data = indy_full, 
          aes(color = grade), 
          fill = NA, #make outlines for redline areas
          linewidth = 1) +
  scale_color_manual(name = "Redlining Grade",
                     values = c("A" = "#76a865",
                                "B" = "#7cb5bd",
                                "C" = "#ffff00",
                                "D" = "#d9838d",
                                "Industrial & Commercial" = "lightgrey"),
                     guide = guide_legend(override.aes = list(fill = NA, size = 2))) + #make square outlines 
  annotation_scale(location = "bl",
                   width_hint = 0.25,
                   unit_category = "imperial", #converts to miles
                   text_cex = 0.8,
                   bar_cols = c("black", "white")) +
  coord_sf(crs = st_crs(indy_full), # forces meters bc it kept being in coordinates
           xlim = c(indy_bbox["xmin"], indy_bbox["xmax"]),
           ylim = c(indy_bbox["ymin"], indy_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(indy_full)) +
  theme_minimal() +
  labs(title = "1. Indianapolis, IN") +
  theme(plot.title = element_text(size = 8.5, 
                                  face = "bold"))
p1

## Pheonix, AR
phe_bbox <- st_bbox(phe_full) # get bounding box in UTM meters

# Overlay projected vector file onto projected NDVI 
p3 <- ggplot() +
  geom_spatraster(data = phe_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale", #use when there's no given scale
                       colours = c("#36454f", "white", "#006400"),
                       limits = c(-1, 1),
                       breaks = c(-0.5, 0, 0.5, 1),
                       labels = c("Low NDVI\n(low vegetation)", "", "", 
                                  "High NDVI\n(high vegetation)"),
                       na.value = "transparent") +
  guides(fill = guide_colorbar(reverse = TRUE)) + #change order scale appears in
  new_scale_color() + #for redlining scale
  geom_sf(data = phe_full, 
          aes(color = grade), 
          fill = NA, #make outlines for redline areas
          linewidth = 1) +
  scale_color_manual(name = "Redlining Grade",
                     values = c("A" = "#76a865",
                                "B" = "#7cb5bd",
                                "C" = "#ffff00",
                                "D" = "#d9838d",
                                "Industrial & Commercial" = "lightgrey"),
                     guide = guide_legend(override.aes = list(fill = NA, size = 2))) + #make square outlines 
  annotation_scale(location = "bl",
                   width_hint = 0.25,
                   unit_category = "imperial", #converts to miles
                   text_cex = 0.8,
                   bar_cols = c("black", "white")) +
  coord_sf(crs = st_crs(phe_full), # forces meters bc it kept being in coordinates
           xlim = c(phe_bbox["xmin"], phe_bbox["xmax"]),
           ylim = c(phe_bbox["ymin"], phe_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(phe_full)) +
  theme_minimal() +
  labs(title = "3. Pheonix, AR") +
  theme(plot.title = element_text(size = 8.5, 
                                  face = "bold"))
p3

## Birmingham, AL
birm_bbox <- st_bbox(birm_full) # get bounding box in UTM meters

# Overlay projected vector file onto projected NDVI 
p4 <- ggplot() +
  geom_spatraster(data = birm_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale", #use when there's no given scale
                       colours = c("#36454f", "white", "#006400"),
                       limits = c(-1, 1),
                       breaks = c(-0.5, 0, 0.5, 1),
                       labels = c("Low NDVI\n(low vegetation)", "", "", 
                                  "High NDVI\n(high vegetation)"),
                       na.value = "transparent") +
  guides(fill = guide_colorbar(reverse = TRUE)) + #change order scale appears in
  new_scale_color() + #for redlining scale
  geom_sf(data = birm_full, 
          aes(color = grade), 
          fill = NA, #make outlines for redline areas
          linewidth = 1) +
  scale_color_manual(name = "Redlining Grade",
                     values = c("A" = "#76a865",
                                "B" = "#7cb5bd",
                                "C" = "#ffff00",
                                "D" = "#d9838d",
                                "Industrial & Commercial" = "lightgrey"),
                     guide = guide_legend(override.aes = list(fill = NA, size = 2))) + #make square outlines 
  annotation_scale(location = "bl",
                   width_hint = 0.25,
                   unit_category = "imperial", #converts to miles
                   text_cex = 0.8,
                   bar_cols = c("black", "white")) +
  coord_sf(crs = st_crs(birm_full), # forces meters bc it kept being in coordinates
           xlim = c(birm_bbox["xmin"], birm_bbox["xmax"]),
           ylim = c(birm_bbox["ymin"], birm_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(birm_full)) +
  theme_minimal() +
  labs(title = "4. Birmingham, AL") +
  theme(plot.title = element_text(size = 8.5, 
                                  face = "bold"))
p4

## Minneapolis, MN
minn_bbox <- st_bbox(minn_full) # get bounding box in UTM meters

# Overlay projected vector file onto projected NDVI 
p5 <- ggplot() +
  geom_spatraster(data = minn_NDVI) +
  scale_fill_gradientn(name = "NDVI Scale", #use when there's no given scale
                       colours = c("#36454f", "white", "#006400"),
                       limits = c(-1, 1),
                       breaks = c(-0.5, 0, 0.5, 1),
                       labels = c("Low NDVI\n(low vegetation)", "", "", 
                                  "High NDVI\n(high vegetation)"),
                       na.value = "transparent") +
  guides(fill = guide_colorbar(reverse = TRUE)) + #change order scale appears in
  new_scale_color() + #for redlining scale
  geom_sf(data = minn_full, 
          aes(color = grade), 
          fill = NA, #make outlines for redline areas
          linewidth = 1) +
  scale_color_manual(name = "Redlining Grade",
                     values = c("A" = "#76a865",
                                "B" = "#7cb5bd",
                                "C" = "#ffff00",
                                "D" = "#d9838d",
                                "Industrial & Commercial" = "lightgrey"),
                     guide = guide_legend(override.aes = list(fill = NA, size = 2))) + #make square outlines 
  annotation_scale(location = "bl",
                   width_hint = 0.25,
                   unit_category = "imperial", #converts to miles
                   text_cex = 0.8,
                   bar_cols = c("black", "white")) +
  coord_sf(crs = st_crs(minn_full), # forces meters bc it kept being in coordinates
           xlim = c(minn_bbox["xmin"], minn_bbox["xmax"]),
           ylim = c(minn_bbox["ymin"], minn_bbox["ymax"]),
           expand = FALSE,
           datum = st_crs(minn_full)) +
  theme_minimal() +
  labs(title = "5. Minneapolis, MN") +
  theme(plot.title = element_text(size = 8.5, 
                                  face = "bold"))
p5
```

```{r}
## Explore the `cowplot` or `patchwork` packages to create a multi-panel figure. 

# Remove legends bc it makes the multi-panel fig look very off
p1_noleg <- p1 + theme(legend.position = "none")
p2_noleg <- p2 + theme(legend.position = "none")
p3_noleg <- p3 + theme(legend.position = "none")
p4_noleg <- p4 + theme(legend.position = "none")
# keep legend for p5
```

```{r}
# install.packages("cowplot")
library(cowplot)

full_plot <- plot_grid(p1_noleg, p2_noleg, p3_noleg,
  p4_noleg, p5, #https://r-graph-gallery.com/package/cowplot.html
  ncol = 3) +
  theme(legend.box.margin = margin(0, 1000000, 0, 0))  # give right column more space 

title <- ggdraw() + 
  draw_label("A", fontface = "bold", size = 14)

# Combine title & full_plot to plot
fig2A <- plot_grid(title, full_plot, ncol = 1, rel_heights = c(0.1, 1))
fig2A
```


### Q7 (2 points)
Now, let's examine the relationship between redlining and NDVI. Temporarily re-read in your redlining polygons using `terra:::vect()`. You can use `terra:extract()` on these temporary polygons within `mutate()` on your original polygons read in with `read_sf()`. Because the output of `terra::extract()` is a data.frame, `dplyr::pull()` can be helpful. 

Extract the mean, median, and central 95% quantile of NDVI from each delineated neighborhood while retaining the identity of the city. Perform your choice of at least two exploratory data visualizations utilizing different variables to evaluate this relationship and examine whether it differs between cities. 

```{r}
## Temporarily re-read in your redlining polygons. Extract the mean, median, and central 95% quantile of NDVI from each delineated neighborhood while retaining the identity of the city. P

#Baltimore
balt_par <- balt_full |> 
  mutate(
    ndvi_mean = terra::extract(balt_NDVI, terra::vect(balt_full), fun = mean, na.rm = TRUE)[, 2],
    ndvi_median = terra::extract(balt_NDVI, terra::vect(balt_full), fun = median, na.rm = TRUE)[, 2],
    ndvi_q95 = {
      ndvi_vals <- terra::extract(balt_NDVI, terra::vect(balt_full)) |> #https://rdrr.io/cran/terra/man/extract.html
        rename(NDVI = 2)
      ndvi_vals |> 
        group_by(ID) |> 
        summarise(q95 = quantile(NDVI, 0.975, na.rm = TRUE) - 
                          quantile(NDVI, 0.025, na.rm = TRUE)) |> 
        pull(q95) #https://www.statology.org/dplyr-pull/
    }
  ) |> 
  mutate(city = "Baltimore")

 head(balt_par)


#Birmingham
birm_par <- birm_full |> 
  mutate(
    ndvi_mean = terra::extract(birm_NDVI, terra::vect(birm_full), fun = mean, na.rm = TRUE)[, 2],
    ndvi_median = terra::extract(birm_NDVI, terra::vect(birm_full), fun = median, na.rm = TRUE)[, 2],
    ndvi_q95 = {
      ndvi_vals <- terra::extract(birm_NDVI, terra::vect(birm_full)) |>
        rename(NDVI = 2)
      ndvi_vals |> 
        group_by(ID) |> 
        summarise(q95 = quantile(NDVI, 0.975, na.rm = TRUE) - 
                          quantile(NDVI, 0.025, na.rm = TRUE)) |> 
        pull(q95)
    }
  ) |> 
  mutate(city = "Birmingham")


#Indianapolis 
indy_par <- indy_full |> 
  mutate(
    ndvi_mean = terra::extract(indy_NDVI, terra::vect(indy_full), fun = mean, na.rm = TRUE)[, 2], 
    ndvi_median = terra::extract(indy_NDVI, terra::vect(indy_full), fun = median, na.rm = TRUE)[, 2],
    ndvi_q95 = {
      ndvi_vals <- terra::extract(indy_NDVI, terra::vect(indy_full)) |>
        rename(NDVI = 2)
      ndvi_vals |> 
        group_by(ID) |> 
        summarise(q95 = quantile(NDVI, 0.975, na.rm = TRUE) - 
                          quantile(NDVI, 0.025, na.rm = TRUE)) |> 
        pull(q95)
    }
  ) |> 
  mutate(city = "Indianapolis")

#Minneapolis
minn_par <- minn_full |> 
  mutate(
    ndvi_mean = terra::extract(minn_NDVI, terra::vect(minn_full), fun = mean, na.rm = TRUE)[, 2],
    ndvi_median = terra::extract(minn_NDVI, terra::vect(minn_full), fun = median, na.rm = TRUE)[, 2],
    ndvi_q95 = {
      ndvi_vals <- terra::extract(minn_NDVI, terra::vect(minn_full)) |>
        rename(NDVI = 2)
      ndvi_vals |> 
        group_by(ID) |> 
        summarise(q95 = quantile(NDVI, 0.975, na.rm = TRUE) - 
                          quantile(NDVI, 0.025, na.rm = TRUE)) |> 
        pull(q95)
    }
  ) |> 
  mutate(city = "Minneapolis")


#Pheonix
phe_par <- phe_full |> 
  mutate(
    ndvi_mean = terra::extract(phe_NDVI, terra::vect(phe_full), fun = mean, na.rm = TRUE)[, 2],
    ndvi_median = terra::extract(phe_NDVI, terra::vect(phe_full), fun = median, na.rm = TRUE)[, 2],
    ndvi_q95 = {
      ndvi_vals <- terra::extract(phe_NDVI, terra::vect(phe_full)) |>
        rename(NDVI = 2)
      ndvi_vals |> 
        group_by(ID) |> 
        summarise(q95 = quantile(NDVI, 0.975, na.rm = TRUE) - 
                          quantile(NDVI, 0.025, na.rm = TRUE)) |> 
        pull(q95)
    }
  ) |> 
  mutate(city = "Pheonix")
```

```{r}
## Perform your choice of at least two exploratory data visualization utilizing different variables to evaluate this relationship and examine whether it differs between cities

balt_par_2 <- st_transform(balt_par, 4326) #transform all to same CRS bc it was giving me issues 
birm_par_2 <- st_transform(birm_par, 4326)
indy_par_2 <- st_transform(indy_par, 4326)
minn_par_2 <- st_transform(minn_par, 4326)
phe_par_2  <- st_transform(phe_par, 4326)

all_cities_par <- bind_rows(balt_par_2, birm_par_2, indy_par_2, minn_par_2, phe_par_2)

# 1. Plot: NDVI v. Redlining across citiies
ggplot(all_cities_par, aes(x = factor(grade, 
                                      levels = c("A", "B", "C", "D")), #each redlining grade
                           y = ndvi_mean, #comparing by ndvi mean 
                           fill = grade)) +
  geom_boxplot() +
  facet_wrap(~ city) +
  scale_fill_manual(name = "Redlining Grade",
                    values = c("A" = "#76a865",
                               "B" = "#7cb5bd",
                               "C" = "#ffff00",
                               "D" = "#d9838d",
                               "Industrial & Commercial" = "lightgrey")) +  
  labs(x = "Redlining Grade",
       y = "Mean NDVI (Vegetation Greenness)") +
  theme_minimal()
```

```{r}
ndvi_summary <- all_cities_par |>
  st_drop_geometry() |> #drops geometry
  group_by(city, grade) |>
  summarise(mean_q95 = mean(ndvi_q95, na.rm = TRUE), .groups = "drop") #finds mean of 95% quantile of NDVI 

# 2. Plot: NDVI Variability v Redlining across cities 
ggplot(ndvi_summary, aes(x = grade, y = mean_q95, fill = grade)) +
  geom_col() +
  facet_wrap(~ city) +
  scale_fill_manual(values = c("A"="#76a865", 
                               "B"="#7cb5bd", 
                               "C"="#ffff00", 
                               "D"="#d9838d")) +
  labs(x = "Redlining Grade",
       y = "Mean NDVI Variability (95% quantile of NDVI)") +
  theme_minimal()
```


### Bonus 2 (1 point)
Perform a statistical test to support your visual analysis above. 
```{r}
## ANOVA models NDVI v. Redlining across cities

# 1.Baltimore 
balt_aov <- aov(ndvi_mean ~ factor(grade), data = balt_par |> 
                  st_drop_geometry())
plot(balt_aov) #checking assumption of normality & variance 

library(car)
leveneTest(ndvi_mean ~ factor(grade), data = balt_par |> 
             st_drop_geometry()) #checking assumption of variance 


# 2.Birmingham
birm_aov <- aov(ndvi_mean ~ factor(grade), data = birm_par |> 
                  st_drop_geometry())
plot(birm_aov) #checking assumption of normality & variance 
leveneTest(ndvi_mean ~ factor(grade), data = birm_par |> 
             st_drop_geometry()) #checking assumption of variance 

# 3.Indianapolis
indy_aov <- aov(ndvi_mean ~ factor(grade), data = indy_par |> 
                  st_drop_geometry())
plot(indy_aov) #checking assumption of normality & variance 
leveneTest(ndvi_mean ~ factor(grade), data = indy_par |> 
             st_drop_geometry()) #checking assumption of variance 

# 4. Minneapolis 
minn_aov <- aov(ndvi_mean ~ factor(grade), data = minn_par |> 
                  st_drop_geometry())
plot(minn_aov) #checking assumption of normality & variance 
leveneTest(ndvi_mean ~ factor(grade), data  =minn_par |> 
             st_drop_geometry()) #checking assumption of variance 



# 5. Pheonix
phe_aov <- aov(ndvi_mean ~ factor(grade), data = phe_par |> 
                  st_drop_geometry())
plot(phe_aov) #checking assumption of normality & variance 
leveneTest(ndvi_mean ~ factor(grade), data  =minn_phe |> 
             st_drop_geometry()) #checking assumption of variance 
```


```{r}
## ANOVA models NDVI v. Redlining across cities

# Split by city and run ANOVA
anova_results <- all_cities_par |> 
  st_drop_geometry() |> 
  group_by(city) |> 
  summarise(anova = list(aov(ndvi_mean ~ factor(grade), data = cur_data()))) |> #use cur_data to drop group by in summary https://stackoverflow.com/questions/62529336/appropriate-times-to-use-cur-data-vs-across-in-dplyr-1-0
  mutate(summary = lapply(anova, summary))

anova_results$summary[[1]] #Baltimore
anova_results$summary[[2]] #Birmingham
anova_results$summary[[3]] #Indianapolis 
anova_results$summary[[4]] #Minneapolis
anova_results$summary[[5]] #Pheonix
```

### Q8 (2 points)
Create a final plot and describe whether NDVI is associated with historical redlining. Does this pattern differ between the five cities examined here? If so, how? 

```{r}
ndvi_summary_2 <- all_cities_par |> 
  st_drop_geometry() |>  # ignore geometry
  group_by(city, grade) |> 
  summarise(
    mean_ndvi = mean(ndvi_mean, na.rm = TRUE),
    median_ndvi = mean(ndvi_median, na.rm = TRUE),
    mean_q95 = mean(ndvi_q95, na.rm = TRUE),
    .groups = "drop")

## Final Plot
ggplot(ndvi_summary_2, aes(x = grade, y = median_ndvi, color = grade)) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = median_ndvi - mean_q95/2, 
                    ymax = median_ndvi + mean_q95/2),
                width = 0.1) +
  facet_wrap(~ city) +
  scale_color_manual(values = c("A"="#76a865", 
                                "B"="#7cb5bd", 
                                "C"="#ffff00", 
                                "D"="#d9838d")) +
  labs(x = "Redlining Grade",
       y = "Median NDVI (95% quantile of NDVI))") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) #diagonal labels & smaller text size
```

According to the final plot comparing redlining grades to the median NDVI with 95% quantile of NDVI across cities, NDVI is associated with historical redlining. Grades A and B overall have a higher median NDVI and less variability across all cities. Grades C and D, have the lowest median NDVI across most of the major cities and greater variability, with Grade D having the lowest median NDVI across all cities except Birmingham. 

### Bonus 3 (1 point)
Include the results of your statistical test in the final plot and use prose to incorporate statistical output in the context of the question above. 

